...
本案例围绕企业经营数据的自然语言查询场景，整理本地数据库查询工程并设计一套本地演示 Schema。底层工程采用 ReAct 思路，将中文自然语言问题转换为 SQL，再执行查询并格式化返回结果。

当前案例为自然语言数据库查询流程演示，使用本地模拟销售业务 Schema，不连接实际企业数据库，也不展示真实业务数据。底层工程面向关系型数据库查询场景，当前示例采用模拟 Schema 展示 Text-to-SQL 流程，覆盖自然语言问题、Schema 感知、SQL 生成、查询结果和结果解释。
...
底层工程面向关系型数据库；当前示例使用模拟销售业务 Schema 和查询样例展示 Text-to-SQL 工作流，后续可接入只读测试库完成进一步运行验证。
...
这些材料基于模拟销售业务数据逻辑，不代表实际企业数据或线上数据库查询结果。
