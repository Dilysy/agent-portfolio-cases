...
本案例面向企业经营数据查询场景，构建了一套自然语言数据库问答流程。用户可以用中文提出经营分析问题，系统结合本地模拟销售业务 Schema 理解数据结构，生成对应 SQL 查询语句，并返回结构化查询结果与解释说明。

当前示例使用本地模拟数据表完成演示，不连接实际企业数据库，也不展示真实业务数据。案例重点展示 Text-to-SQL 在企业数据分析场景中的应用流程，包括自然语言问题理解、Schema 感知、SQL 生成、查询执行、结果返回和人工复核边界。
