...
本案例面向团队知识管理产品的竞品分析场景，构建了一套从业务问题输入、公开信息检索、关键信息提取到 Markdown 报告生成的自动化分析流程。系统通过 Jupyter Notebook 展示快速工具调用 Agent 与 PlanAndSolveAgent 两种执行范式，便于对比不同任务规划方式在信息分析场景中的适用性。

当前示例基于公开信息和本地演示环境完成验证，重点展示 Agent 在信息搜索、结构化整理和报告生成中的流程编排能力。正式使用前，仍需对信息来源、事实准确性和分析结论进行人工复核。
...
- 当前已完成 LLM 与公开搜索服务的联动验证，并生成竞品分析报告。正式使用时仍必须对指定来源进行人工复核。
...
| 内容清洗 | 部分包含 | SimpleAgent 版本为 原型示例；PlanSolve 版本 v2.0 使用规则和正则抽取结构化字段。 |
...
| --- | --- | --- | --- | --- |
| LLM 客户端 | 用户任务、系统提示词、工具执行上下文 | Agent 推理结果、工具调用决策 | 驱动 Agent 理解任务、规划步骤和生成文本 | 已完成本地运行验证，需配置本地模型服务参数 |
...
| `CompetitiveInfoSearchTool` | 单个产品名称 | 搜索结果或失败提示 | 查询产品功能、定价、优缺点等公开信息 | 已完成公开搜索验证，需配置本地搜索服务参数和可用网络 |
| `DataProcessorTool` | 搜索返回文本 | 产品名称、定位、功能、定价、优势、劣势等结构化字段 | 清洗和结构化竞品信息 | PlanSolve v2.0 可用；SimpleAgent 版本为 原型示例 |
| `ReportGeneratorTool` | 结构化产品字段列表 | Markdown 竞品分析报告 | 生成对比表、分析结论和建议 | PlanSolve v2.0 可用；正式使用需人工复核 |
| 输出保存逻辑 | Agent 最终报告 | `assets/reports/competitor-analysis-agent/competitor-analysis-report.md` | 保存示例报告，便于归档和复盘 | 已有输出样例 |

## 6. 实现流程

1. 在工程目录安装依赖，并按模板准备本地配置文件。
2. 配置本地模型服务与搜索服务运行参数。
...
7. 报告生成工具输出 Markdown 报告，并归档到案例报告目录。
8. 人工复核来源、事实口径、建议合理性和是否存在敏感信息。

## 7. 输出结果

本阶段已完成一次本地运行验证，模型服务与公开搜索服务均调用成功，并生成运行报告：
...
报告基于公开搜索结果和模型生成能力整理，用于说明 Agent 输出结构和案例组织方式。正式使用前仍需要结合指定来源复核，尤其是价格、套餐、AI 功能和企业版权限等易变化信息。
...
- 风险检查：确认输出中没有本地配置、账号、邮箱、微信、浏览器隐私页或敏感业务信息。
