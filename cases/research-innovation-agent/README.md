...
科研资料整理通常涉及论文检索、摘要阅读、方法对比、创新点提炼、综述段落撰写和引用格式校验等多个环节。若这些任务分散在不同工具中完成，研究人员需要频繁切换上下文，整理效率较低，输出结果也不易统一管理。

本案例面向科研资料整理与论文辅助分析场景，构建了一套可本地运行的科研创新助手 Agent 流程。系统通过 Hunter、Miner、Coach、Validator 四类 Agent 分别承担论文检索、论文分析、写作建议和引用校验任务，展示从研究主题输入到资料获取、内容理解、表达优化和引用规范化的完整辅助链路。
## 2. 业务问题
