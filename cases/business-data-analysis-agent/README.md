...
企业经营分析通常需要围绕销售额、区域表现、产品结构、客户类型和时间趋势等指标进行持续跟踪。传统分析方式往往依赖人工整理表格、计算指标、制作图表和撰写报告，流程重复且容易受到数据口径、统计方法和报告模板不一致的影响。

本案例面向企业经营数据分析场景，构建了一套本地化数据分析 Agent 流程。系统以示例经营数据为输入，完成指标计算、分组对比、趋势分析、图表生成和 Markdown 报告输出，用于展示从数据处理到经营洞察生成的自动化分析链路。
围绕企业经营表格分析场景进行工程化封装和流程编排整理，综合两个数据分析 Agent 项目的能力边界：
...
当前示例基于本地演示数据完成验证，不接入实际企业数据库，也不代表真实经营结果。正式应用前，仍需结合业务口径、数据质量和管理要求进行人工复核。
...
| 指标计算 | 多个固定分析函数计算购物行为指标 | DataStatisticsTool 做描述性统计 | 已包含，但 多 Agent 原型与固定数据集强绑定 |
...
| 多 Agent 分工 | PlanningAgent / AnalysisAgent / ReportAgent | 单 Agent 为主 | 多 Agent 原型可作为多 Agent 分工展示 |

需要注意：`多 Agent 原型` 的分析工具主要围绕 `shopping_behavior_updated.csv` 固定字段设计，泛化到任意经营表格需要改造。`Notebook 原型` Notebook 中存在硬编码模型环境变量配置，展示前必须清理。

## 5. 工具调用

| 工具名称 | 输入 | 输出 | 作用 | 当前可用状态 |
| --- | --- | --- | --- | --- |
| `get_basic_metadata` | CSV 数据 | 行列数、字段名、数据类型、内存占用 | 数据理解和字段识别 | 多 Agent 原型 中已实现 |
| `assess_data_quality` | CSV 数据 | 缺失率、异常值、有效性检查 | 数据质量评估 | 多 Agent 原型 中已实现 |
| `get_statistical_summary` | 数值字段 | 均值、分位数、偏度、峰度、变异系数 | 统计摘要 | 多 Agent 原型 中已实现 |
| `Gender Preference Analysis` | 分析子任务 | 性别分布、平均消费、品类偏好和图表路径 | 客户分群分析 | 多 Agent 原型 中已实现，面向固定购物数据 |
| `Age Preference Analysis` | 分析子任务 | 年龄段分布、消费金额、品类偏好和图表路径 | 年龄分层分析 | 多 Agent 原型 中已实现，面向固定购物数据 |
| `Spending Differences Analysis` | 分析子任务 | 性别/年龄/品类交叉消费差异和图表路径 | 交叉对比分析 | 多 Agent 原型 中已实现，面向固定购物数据 |
| `Subscription Impact Analysis` | 分析子任务 | 订阅状态、消费金额、复购频率和图表路径 | 订阅影响分析 | 多 Agent 原型 中已实现，面向固定购物数据 |
| `Seasonal Preference Analysis` | 分析子任务 | 季节偏好、热销品类和图表路径 | 季节性分析 | 多 Agent 原型 中已实现，面向固定购物数据 |
| `Review Rating Impact Analysis` | 分析子任务 | 评分与购买金额/频率关系 | 评价影响分析 | 多 Agent 原型 中已实现，面向固定购物数据 |
...
8. 在文档中标注当前状态：模拟数据、图表、报告样例和演示视频已完成，底层工程端到端运行验证作为后续增强。
...
6. 清理 Notebook 原型 中的硬编码敏感配置，改为 `本地配置文件` 加载。
