...
在日常开发和项目维护过程中，代码审查通常需要关注潜在缺陷、异常处理、重复逻辑、资源释放、配置安全和可维护性等多个方面。传统人工审查依赖经验判断，面对较多文件或重复性检查任务时，容易出现审查口径不一致、问题定位耗时较长和修复建议不够结构化的问题。

本案例面向代码审查场景，构建了一套基于本地代码样例的智能审查 Agent 流程。系统读取待审查代码后，对代码结构、潜在问题和风险点进行分析，输出问题清单、风险等级、代码位置和修复建议，并通过结构化报告方式呈现，便于后续人工复核和修正。

当前示例基于本地演示代码完成验证，不接入真实业务代码仓库，也不代表生产级代码审计结果。正式使用前，仍需结合项目上下文、编码规范、安全要求和人工审查结论进行复核。
...
本案例将代码审查能力构建为 Agent 演示流程：

1. 使用 CLI 指定待审查仓库或文件目录。
2. Agent 通过 ReAct 循环按需调用工具读取文件、搜索上下文、记录任务进度。
3. LLM 根据代码片段生成审查意见，包括问题定位、风险等级和修复建议。
4. 对需要修改的内容，Agent 可生成 补丁格式补丁，并由 CLI 执行确认与落盘。
5. 当前示例生成 Markdown 审查报告和 HTML 预览页，便于录屏展示。

本案例已完成真实 LLM 调用验证，并通过真实代码审查调用链路生成审查输出。当前定位是本地验证与演示验证，不写成生产级代码审计平台或实际客户项目。
...
| 报告生成 | 可由 LLM 生成 | 当前审查流程没有固定报告模板；本案例补充了 Markdown 与 HTML 样例。 |
...
| `ApplyPatchExecutor` | 补丁格式 patch | 修改文件、备份路径 | 安全应用补丁，包含路径限制、备份和规模限制 | 已实现 |
| `LLMClient` | 本地模型配置和消息 | 模型回复 | 生成代码解释、审查结论和修复建议 | 已完成真实调用验证；配置内容不进入本案例 |
...
当前演示材料用于展示本地验证、真实调用验证和报告整理流程，不代表实际客户项目结果。
