...
| 结尾 | 3 秒 | `standard-mode.png` 或四模式截图拼接 | 总结本地化运行验证、LLM 接入、四模式验证。 | 静态图，叠加简短结尾文案。 |
...
- 是否存在报错、终端、Codex、桌面隐私、私密凭证、账号、邮箱、微信等画面。
...
- 不保留报错、终端、Codex、桌面隐私、私密凭证、账号、邮箱、微信等画面。
...
- 复核说明：本成片用于展示 AutoFlow 的本地化运行验证、LLM 接入、四模式验证、实时预览和导出能力，不写成真实客户交付项目。
