...
本记录不包含任何 API Key，也未读取或记录真实 `.env` 内容。
...
1. `backend/src/config.py`
   - 增加 `llm_timeout`，支持 `LLM_TIMEOUT`。
   - 增加 `max_todo_items`，支持 `MAX_TODO_ITEMS`。
...
1. 当前后端若直接使用 `PYTHONPATH=backend/src backend/.venv/bin/python backend/src/main.py` 启动，原入口没有在本地模块导入前自动加载 `backend/.env`。
2. 未加载 `.env` 时，LLM 配置会回落到默认 `ollama` 和本地地址，导致前端看到 `Connection error`。
3. 已将 `backend/src/main.py` 调整为在导入本地 Agent 模块前加载 `backend/.env`，并将启动日志中的 API Key 显示方式改为仅显示 `set/unset`。
...
- 脚本只输出成功状态、base_url/model 是否为空、HTTP 状态码或错误类型、耗时、返回文本前 20 个字符和错误摘要。
- 健康检查结果：LLM 请求未成功，`base_url` 非空，`model` 非空，错误类型为 `PermissionDeniedError`，HTTP 状态码为 `403`。
...
1. 检查后端 `.env` 中的 `LLM_API_KEY` 对应账户是否仍有可用额度。
2. 如更换模型服务或账号，重点检查 `LLM_API_KEY`、`LLM_BASE_URL`、`LLM_MODEL_ID` 三个字段。
3. 修改 `.env` 后重新启动后端，再运行 `check_llm_health.py` 做最小验证。
...
- 本次采样在 120 秒内未收到 `task_summary_chunk`，说明任务总结为空主要与 Summarizer 尚未产出、请求被中断或任务失败有关。
...
- 用户已配置 Tavily Key，希望优先使用 Tavily。

排查结论：

1. 后端搜索后端由 `Configuration.search_api` 决定，默认值原为 `duckduckgo`。
2. 前端搜索引擎下拉框默认是“沿用后端配置”，如果用户没有在前端显式选择 `tavily`，前端不会覆盖后端配置。
3. 原逻辑中，仅配置 `TAVILY_API_KEY` 不会自动把 `search_api` 切换为 `tavily`；必须同时配置 `SEARCH_API=tavily`，否则可能继续使用默认 DuckDuckGo。
4. 本次安全检查显示当前本地配置加载后 `search_api=tavily`，Tavily Key 状态为 set；如果页面仍提示 DuckDuckGo，通常说明后端没有重启到新配置，或前端下拉框显式选择了 `duckduckgo`。

本次最小修复：

- `backend/src/config.py`：当没有显式 `SEARCH_API`，但检测到 `TAVILY_API_KEY` 时，自动推断 `search_api=tavily`。
- 若没有 Tavily Key，但存在 `PERPLEXITY_API_KEY` 或 `SEARXNG_URL`，也会按对应搜索后端推断。
- 显式设置仍优先，例如 `SEARCH_API=duckduckgo` 会继续尊重用户选择。
- `backend/.env.example`：将推荐搜索后端写为 `SEARCH_API=tavily`。

验证结果：

- 模拟无 `SEARCH_API`、有 Tavily Key 时，配置推断结果为 `tavily`。
- 显式设置 `SEARCH_API=duckduckgo` 时，仍使用 `duckduckgo`。
...
1. 后端 `.env` 建议明确配置 `SEARCH_API=tavily`。
